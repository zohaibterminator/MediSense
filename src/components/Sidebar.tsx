...
export const Sidebar = ({ 
  ...
  const fetchChats = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/chat/?user_id=${userId}`);
  ...
  const addNewChat = async (chatName: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/chat/create`, {
        method: 'POST',
        headers: {
  ...
  const deleteChat = async (chatId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/chat/delete/${chatId}`, {
        method: 'DELETE',
      });
  ...
  const handleRename = async (chatId: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/chat/update/${chatId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
